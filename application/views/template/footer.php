...
<script>
	...
	$('.tipeuserAdd').change(function() {
		...
		if ($(this).val() != 'salah') {
			...
			if (parseInt($(this).val()) == 1) {
				...
				$.get(baseUrl + 'bulanan/getBulanan', function(result) {
					let data = JSON.parse(result);
					// console.log(data)
					$('.cusName').html('');
					$('.cusName').removeAttr('disabled');
					$('.cusName').append('<option value="">Pilih Bulanan</option>')
					data.forEach(function(res) {
						$('.cusName').append('<option value="' + res.id_bulananlaporan + '">' + res.bulananlaporan + ' (' + res.bulananlaporan + ')</option>')
					})
				});
				$.get(baseUrl + 'mtransaksi/getMTransaksiBulanan/bulanan', function(result) {
					let data = JSON.parse(result);
					// console.log(data)
					$('.kategori').html('');
					$('.kategori').removeAttr('disabled');
	...
	$('#laporanTanggalLaporan').change(function() {
		let idTanggalLaporan = this.value;
		console.log(idTanggalLaporan)
		if (this.value !== 'Pilih Tanggal') {
			$.get(baseUrl + 'laporan/getLaporanByTanggalLaporan/' + this.value, function(result) {
				let data = JSON.parse(result);
				$('#dataLaporanTanggalLaporan').html(' ');
				// $('#selectKeca').removeAttr('disabled');
				if (data.length === 0 || data === null || data === undefined) {
					$('#dataLaporanTanggalLaporan').append('<tr><td colspan="7" align="center">Tidak Ada Data</td></tr>');
					$('.btn-grad').attr('disabled', 'disabled');
				} else {
					let i = 1;
					$('.btn-grad').removeAttr('disabled');
					$('.btn-grad').click(function() {
						if (confirm('Yakin')) {
							window.location.href = baseUrl + 'laporan/export_process/' + idTanggalLaporan;
						}
					})
					let tanggallaporan = $('#laporanTanggalLaporan option:selected').html()
					data.forEach(function(res) {
						$('#dataTanggalLaporan').append('<tr><td>' + i + '</td><td>' + res.nama_produk + '</td><td>' + res.nama_kategpri + '</td><td>' + res.jumlah + '</td><td>' + res.laba + '</td><td>' + res.nilai + '</td></tr>');
						i++;
					})
				}
			});
	...
	$("#btnImportS").click(function() {
		if (confirm('Yakin Import Data')) {
			window.location.href = baseUrl + 'laporan/import?id_tanggallaporan=' + $("#ta").val();
		}
	})
